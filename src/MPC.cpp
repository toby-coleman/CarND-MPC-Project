...
//
MPC::MPC(double target_velocity, int delay_steps) {
  x_predict.resize(N);
  y_predict.resize(N);
  this->ref_v_ = target_velocity;
  this->delay_ = delay_steps;
}
MPC::~MPC() {}
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set constraints for throttle and steering during actuation delay
 for (int i = a_start; i < a_start + delay_; i++) {
    vars_lowerbound[i] = throttle_;
    vars_upperbound[i] = throttle_;
 }
 for (int i = delta_start; i < delta_start + delay_; i++) {
  ...
  // Copy forward trajectory to public variables
  for (int i = 0; i < N; i++) {
    x_predict[i] = solution.x[x_start + i];
    y_predict[i] = solution.x[y_start + i];
  }
  ...
  throttle_ = solution.x[a_start + delay_];
  steer_ = solution.x[delta_start + delay_];
  return {steer_, throttle_};
}
